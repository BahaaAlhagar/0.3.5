...
bool TalkActions::onPlayerSay(Creature* creature, uint16_t channelId, const std::string& words, bool ignoreAccess)
{
	...
	for(TalkActionsMap::iterator it = talksMap.begin(); it != talksMap.end(); ++it)
	{
		if(it->first != "illegalWords" && (it->first == cmd[it->second->getFilter()] ||
			(!it->second->isSensitive() && boost::algorithm::iequals(it->first, cmd[it->second->getFilter()]))))
		{
	...
	if(!talkAction)
	{
		TalkActionsMap::iterator it = talksMap.find("illegalWords");
		if(it != talksMap.end()) {
			return (it->second && it->second->executeSay(creature, words, "", channelId));
		}
		return false;
	}
...
bool TalkAction::ghost(Creature* creature, const std::string& cmd, const std::string& param)
{
	...
	g_game.updateCreatureWalkthrough(player);
