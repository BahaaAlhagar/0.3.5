...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	{
		if((*it)->isPersistent() || (*it)->getType() == CONDITION_GAMEMASTER || (*it)->getType() == CONDITION_EXHAUST)
		{
	...
	query.str("");
	query << "DELETE FROM `player_skills` WHERE `player_id` = " << player->getGUID();
	if(!db->executeQuery(query.str()))
		return false;

	char buffer[280];
	DBInsert query_insert(db);
	query_insert.setQuery("INSERT INTO `player_skills` (`player_id`, `skillid`, `value`, `count`) VALUES ");
	for(int32_t i = SKILL_FIRST; i <= SKILL_LAST; ++i)
	{
		sprintf(buffer, "%u, %d, %u, %u", player->getGUID(), i, player->skills[i][SKILL_LEVEL], player->skills[i][SKILL_TRIES]);
