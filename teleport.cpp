...
void Teleport::__addThing(Creature* actor, int32_t index, Thing* thing)
{
	...
	{
		g_game.addMagicEffect(creature->getPosition(), MAGIC_EFFECT_ENERGY_AREA, creature->isGhost());
		g_game.internalCreatureTurn(creature, getDirectionTo(creature->getPosition(), destTile->getPosition(), false));
		creature->getTile()->moveCreature(actor, creature, destTile);
		g_game.addMagicEffect(destTile->getPosition(), MAGIC_EFFECT_ENERGY_AREA, creature->isGhost());
	}
	else if(Item* item = thing->getItem())
	{
		g_game.addMagicEffect(item->getPosition(), MAGIC_EFFECT_ENERGY_AREA);
		g_game.internalMoveItem(actor, item->getTile(), destTile, INDEX_WHEREEVER, item, item->getItemCount(), NULL, FLAG_NOLIMIT);
		g_game.addMagicEffect(destTile->getPosition(), MAGIC_EFFECT_ENERGY_AREA);
	}
}
