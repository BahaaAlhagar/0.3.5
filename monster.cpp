...
void Monster::onThinkDefense(uint32_t interval)
{
	...
	if(!isSummon())
	{
		if(mType->maxSummons < 0 || (int32_t)summons.size() < mType->maxSummons)
		{
			...
			{
				if(!getAttackedCreature() || (int32_t)summons.size() >= mType->maxSummons)
					break;
...
Item* Monster::createCorpse(DeathList deathList)
{
	...
	DeathEntry ownerEntry = deathList[(deathList.size() != 1 && deathList[1].getDamage() > deathList[0].getDamage()) ? 1 : 0];
