...
void Game::saveGameState(bool shallow)
{
	...
	std::cout << "> SAVE: Players saved in " << OTSYS_TIME() - start << " ms." << std::endl;
	...
	start = OTSYS_TIME();
	map->saveMap();
	std::cout << "> SAVE: Houses saved in " << OTSYS_TIME() - start << " ms." << std::endl;
	...
	const Position& movingCreaturePos, const Position& toPos)
{
	...
	if(player != movingCreature)
	{
		...
		if(Npc* npc = movingCreature->getNpc())
		{
			if(!Spawns::getInstance()->isInZone(npc->masterPosition, npc->masterRadius, toPos))
			{
				player->sendCancelMessage(RET_NOTENOUGHROOM);
				return false;
			}
	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
	...
	if(player->hasCondition(CONDITION_EXHAUST, EXHAUST_MOVE))
		return false;
	player->addExhaust(200, EXHAUST_MOVE);
	if(!player->canDoAction())
	{
		uint32_t delay = player->getNextActionTime();
...
ReturnValue Game::internalTeleport(Thing* thing, const Position& newPos, bool pushMove, uint32_t flags /*= 0*/)
{
	...
	if(Tile* toTile = map->getTile(newPos))
	{
		...
		{
			creature->getTile()->moveCreature(NULL, creature, toTile, (pushMove && Position::areInRange<1,1,0>(creature->getPosition(), newPos)) ? false : true);
			if(!pushMove)
			{
...
bool Game::playerTurn(uint32_t playerId, Direction dir)
{
	...
	if(player->hasCondition(CONDITION_EXHAUST, EXHAUST_TURN))
		return false;

	if(internalCreatureTurn(player, dir))
	{
		player->setIdleTime(0);
		player->addExhaust(250, EXHAUST_TURN);
		return true;
	}
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClasses type, const std::string& receiver, const std::string& text)
{
	...
	if(ret == RET_NOERROR)
		...
		if(g_config.getBool(ConfigManager::BUFFER_SPELL_FAILURE))
			player->removeMessageBuffer();
		return true;
	}
		...
		if(damage != 0)
		{
			if(target->hasCondition(CONDITION_MANASHIELD) && combatType != COMBAT_UNDEFINEDDAMAGE)
			{
				...
				if(manaDamage != 0)
				{
					...
					if(combatType == COMBAT_PHYSICALDAMAGE && attacker && attacker->getPlayer() && attacker->hasCondition(CONDITION_ATTRIBUTES, 6))
						manaDamage *= random_range(109, 112) / 100;
	...
	std::string comment, std::string statement, uint32_t statementId, bool ipBanishment)
{
	...
	std::stringstream ss;
	//if(reason != 6 && g_config.getBool(ConfigManager::BROADCAST_BANISHMENTS))
	...
	if(ChatChannel* channel = g_chat.getChannelById(CHANNEL_STAFF)) {
		for(UsersMap::const_iterator it = channel->getUsers().begin(); it != channel->getUsers().end(); ++it)
			it->second->sendChannelMessage("", ss.str(), SPEAK_CHANNEL_RN, CHANNEL_STAFF);
	}
