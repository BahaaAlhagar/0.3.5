...
void LuaScriptInterface::registerFunctions()
{
	...
	lua_register(m_luaState, "getConditionEnd", LuaScriptInterface::luaGetConditionEnd);
...
{
	//doTeleportThing(cid, newpos[, pushmove = false])
	bool pushMove = false;
	if(lua_gettop(L) > 2)
...
int32_t LuaScriptInterface::luaGetConditionEnd(lua_State* L)
{
	//getConditionEnd(cid, condition[, subId])
...
{
	//getTownTemplePosition(townId)
	if(Town* town = Towns::getInstance()->getTown(popNumber(L)))
...
int32_t LuaScriptInterface::luaGetHouseItemCount(lua_State* L)
{
	//getHouseItemCount(houseId)
	uint32_t houseId = popNumber(L);
	if(House* house = Houses::getInstance()->getHouse(houseId))
	{
		lua_pushnumber(L, house->getItemCount());
	}
	else
		lua_pushboolean(L, false);
