...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	if((((g_game.getWorldType() == WORLD_TYPE_NO_PVP &&
		...
		player->getTile()->ground->getID() != 11060) && (!player->hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges)
...
{
	if(!internal) {
		if(hasCondition(CONDITION_EXHAUST, EXHAUST_VIP))
			return false;

		addExhaust(1000, EXHAUST_VIP);
	}
	if(guid == _guid)
	{
		if(!internal)
...
bool Player::onKilledCreature(Creature* target, DeathEntry& entry)
{
	...
	/*if(hasFlag(PlayerFlag_NotGenerateLoot) == true)
...
{
	if(party && party->isSharedExperienceEnabled() && party->isSharedExperienceActive() && !target->getPlayer())
	{
...
bool Player::canWearOutfit(uint32_t outfitId, uint32_t addons)
{
	...
	if(it == outfits.end() || (it->second.isPremium && !isPremium()) || (it->second.accessLevel != 0 && getAccess() != it->second.accessLevel)
